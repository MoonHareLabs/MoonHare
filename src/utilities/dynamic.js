...
import { generatePlaceholder, generateFontSize, expandDirection, } from '../../utils/helpers';
...
function gridRow(utility, { theme }) {
    ...
    if (utility.raw.startsWith('row-span')) {
        return utility.handler
            .handleNumber(1, undefined, 'int')
            .handleVariable()
            .createProperty(['-ms-grid-row-span', 'grid-row'], (value) => `span ${value} / span ${value}`)
...
function size(utility, { theme }) {
    ...
    return utility.handler
        ...
        .handleFraction()
        .handleSize()
        .handleNxl((number) => `${(number - 3) * 8 + 48}rem`)
        .handleVariable()
        .createProperty(name)
    ...
    if (utility.raw.startsWith('text-shadow')) {
        return (utility.raw === 'text-shadow'
            ...
            : utility.handler
                ...
                .createProperty('text-shadow'))?.updateMeta('utilities', 'textShadow', pluginOrder.textShadow, 1, true);
    }
...
function lineHeight(utility, { theme }) {
    return utility.handler
        ...
        .handleNumber(0, undefined, 'int', (number) => `${number * 0.25}rem`)
        .handleSquareBrackets()
        .handleSize()
        .handleVariable()
...
function placeholder(utility, { theme, config }) {
    ...
    if (color)
        return generatePlaceholder(color.selector || '', color.property, config('prefixer')).map(i => i.updateMeta('utilities', 'placeholderColor', pluginOrder.placeholderColor, 2, true));
}
...
function background(utility, { theme }) {
    ...
    if (Object.keys(images).includes(body)) {
        ...
        if (Array.isArray(prefixer))
            return new Style(utility.class, prefixer.map((i) => new Property('background-image', i))).updateMeta('utilities', 'backgroundImage', pluginOrder.backgroundImage, 2, true);
...
function gradientColorVia(utility, { theme }) {
    ...
    const handler = utility.handler.handleColor(theme('gradientColorStops')).handleOpacity(theme('opacity')).handleVariable().handleSquareBrackets();
    if (handler.color || handler.value) {
        return new Style(utility.class, new Property('--tw-gradient-stops', `var(--tw-gradient-from), ${handler.createColorValue('var(--tw-via-opacity, 1)')}, var(--tw-gradient-to, rgba(255, 255, 255, 0))`))?.updateMeta('utilities', 'gradientColorStops', pluginOrder.gradientColorStops, 3, true);
    }
}
...
function gradientColorTo(utility, { theme }) {
    ...
    const handler = utility.handler.handleColor(theme('gradientColorStops')).handleOpacity(theme('opacity')).handleVariable().handleSquareBrackets();
    if (handler.color || handler.value) {
        return new Style(utility.class, new Property('--tw-gradient-to', handler.createColorValue('var(--tw-to-opacity, 1)')))?.updateMeta('utilities', 'gradientColorStops', pluginOrder.gradientColorStops, 5, true);
    }
}
...
function borderRadius(utility, { theme }) {
    ...
    return utility.handler
        ...
        .handleSquareBrackets()
        .handleFraction()
        .handleNxl((number) => `${number * 0.5}rem`)
        .handleSize()
        .handleVariable()
...
function border(utility, { theme }) {
    ...
    return utility.handler
        ...
        .handleSquareBrackets()
        .handleNumber(0, undefined, 'int', (number) => /^border(-[tlbr])?$/.test(utility.key) ? `${number}px` : undefined)
        .handleSize()
        .handleVariable()
...
function divide(utility, { theme }) {
    ...
    const divideColor = utility.handler
        ...
        .handleVariable((variable) => utility.raw.startsWith('divide-$') ? `var(--${variable})` : undefined)
...
function ring(utility, utils) {
    ...
    const ringColor = utility.handler
        ...
        .handleSquareBrackets(notNumberLead)
        .handleVariable((variable) => utility.raw.startsWith('ring-$') ? `var(--${variable})` : undefined)
...
function dropShadow(utility, { theme }) {
    ...
    if (value)
        return new Property('--tw-drop-shadow', Array.isArray(value) ? value.map(i => `drop-shadow(${i})`).join(' ') : `drop-shadow(${value})`).updateMeta('utilities', 'dropShadow', pluginOrder.dropShadow, 1, true);
}
...
function duration(utility, { theme }) {
    return utility.handler
        ...
        .handleNumber(0, undefined, 'int', (number) => `${number}ms`)
        .handleVariable()
    ...
    if (Object.keys(animations).includes(body)) {
        ...
        if (styles) {
            return [
                ...
                ...keyframe ? Keyframes.generate(keyframe, (theme(`keyframes.${keyframe}`) ?? {}), undefined, config('prefixer', false)).map(i => i.updateMeta('utilities', 'keyframes', pluginOrder.keyframes, 1, true)) : [],
            ];
        }
    }
}
